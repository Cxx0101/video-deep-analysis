...
def _make_pretrained_efficientnet_lite3(use_pretrained, exportable=False):
    # MiDaS weights include the backbone parameters, so no ImageNet download is
    # required. timm replaces the original torch.hub repository dependency.
    efficientnet = timm.create_model("tf_efficientnet_lite3", pretrained=use_pretrained, exportable=exportable)
...
def _make_efficientnet_backbone(effnet):
    ...
    pretrained.layer1 = nn.Sequential(
        # Recent timm versions fold this activation into BatchNormAct2d. Keep an
        # Identity placeholder so checkpoint module indices remain unchanged.
        effnet.conv_stem, effnet.bn1, getattr(effnet, "act1", nn.Identity()), *effnet.blocks[0:2]
    )
